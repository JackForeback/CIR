"""Figures for the original linear-classifier script.

Part of the *original*, pre-refactor codebase; see ``README.md``.

Every function takes an explicit ``path`` for its output directory. The
pre-refactor version read a module-level ``path`` from ``sys.argv[2]`` at import
time, which crashed on import unless the caller happened to pass ``--path``
first. Matplotlib is pinned to the ``Agg`` backend so the script runs headless on
a compute node.
"""
...
def plot_samples(data, num_classes, samples_per_class, path):
    """
...
def make_animation(seeds, training_steps, path):
    """
    ...
    Args:
        ...
        path (str): Output directory; must contain db/ and will receive ani/.
    """
    ...
    for i in range(seeds):
        ...
        # Load all images that were actually written
        frames = [Image.open(p) for p in image_paths if os.path.exists(p)]
        if not frames:
            continue

        os.makedirs(f"{path}/ani", exist_ok=True)
...
def plot_avg_accuracy(train, test, num_classes, path):
    """
    ...
    Args:
        ...
        num_classes (int): Number of classes, used to pick the colormap.
        path (str): Output directory.
    """
...
def seed_plot(per_seed, num_seeds, num_training_steps, path):
    """
    ...
    Args:
        ...
        path (str): Output directory; figures land in path/seed/.
    """
...
def plot_decision_boundaries(data, Y, classes, num_classes, weights, biases, step, seed, path):
    """
    ...
    Args:
        ...
        path (str): Output directory; frames land in path/db/.
    """
    plt.figure(figsize=(10, 6))
    cmap = plt.get_cmap('tab10' if num_classes <= 10 else 'nipy_spectral', num_classes)
    x_vals = torch.linspace(data[:, 0].min() - 1, data[:, 0].max() + 1, 500)

    # Plot samples. Uses a vectorized argmax mask rather than a per-sample
    # torch.equal comparison, which made every animation frame O(N * classes).
    labels = torch.stack(list(Y)).argmax(dim=1)
    for class_id in range(num_classes):
    ...
    for i in range(num_classes):
        for j in range(i + 1, num_classes):
            ...
            if b != 0:
                ...
                x_intercept = -c / a
                plt.axvline(x=x_intercept, linewidth=1)
            else:
...
def format_plot(title="", xlabel="X", ylabel="Y", save_path="plot.png"):
    """
    ...
    Args:
        ...
        save_path (str): Path to save the plot image. Parent directories are
